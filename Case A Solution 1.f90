...
Subroutine calc
  ...
  d=0
  ! X=p
  Constn(1)=p*r/(1d0+kappa)*(0,1)
  Constp(1)=-kappa*p*r/(1d0+kappa)*(0,1)
  Constp(3)=-2d0*(1-r**(-2))*r**2*p*r/(1d0+kappa)*(0,1)
  ...
  write(*,*) 'Total iteration reps of Case A =', q

  open(2001,file=datapath//'/dk/case_a_solution1.csv')
  do k=-10,10
...
20001 format(I4,A,F10.6,A,F10.6)
  ...
  open(1001,file=datapath//'/case_a/stress_case_a.csv')
  do i=1,T+1
  ...
  open(1002,file=datapath//'/case_a/displacement_case_a.csv')
  do i=1,T+1
     write(1002,10002) 1d0*(i-1)/T*360, ',', &
          -(ux(i)-ux(T))*10, ',', -(uxr(i)-ux(T))*10, ',', -(uxrho(i)-ux(T))*10, ',', &
          -(uy(i)-uy(T))*10, ',', -(uyr(i)-uy(T))*10, ',', -(uyrho(i)-uy(T))*10
  enddo
  close(1002)
  ! do i=1,T
  !    write(*,*) g(i),gr(i),grho(i)
  ! enddo
  write(*,*) 'Horizontal rigid-body displacement of Case A Sol 1 =', ux(T)*1e1
  write(*,*) 'Vertical rigid-body displacement of Case A Sol 1 =', uy(T)*1e1
