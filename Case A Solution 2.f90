Module Parameters
  ...
  real(8),parameter::theta2=0d0/180*3.1415926535897932384626433832795
  real(8),parameter::nu=0.3
  real(8),parameter::pi=3.1415926535897932384626433832795
  integer,parameter::N=60
  integer,parameter::M11=3e4,M12=4e5
...
Subroutine calc
  ...
  enddo
  open(1010,file=datapath//'/ck12/ck12_case_a.csv')
  do l=1,N
     write(1010,20002) l, ',' &
...
20002 format(I4,A,F10.6,A,F10.6,A,F10.6,A,F10.6)
  ...
  open(2001,file=datapath//'/dk/case_a_solution2.csv')
  do k=-10,10
...
20001 format(I4,A,F10.6,A,F10.6)
  ...
  open(1001,file=datapath//'/case_a/stress_case_a2.csv')
  do i=1,T+1
  ...
  open(1002,file=datapath//'/case_a/displacement_case_a2.csv')
  do i=1,T+1
  ...
  ! enddo
  write(*,*) 'Horizontal rigid-body displacement of Case A Sol 2 =', ux(T)*1e1
  write(*,*) 'Vertical rigid-body displacement of Case A Sol 2 =', uy(T)*1e1
