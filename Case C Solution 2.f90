...
Subroutine calc
  ...
  enddo
  open(1010,file=datapath//'/ck12/ck12_case_c.csv')
  do l=1,N
     write(1010,20002) l, ',' &
...
20002 format(I4,A,F10.6,A,F10.6,A,F10.6,A,F10.6)
  ...
  write(*,*) 'Error of', q, 'th iteration =',err
  open(2001,file=datapath//'/dk/case_c_solution2.csv')
  do k=-N,N
...
20001 format(I4,A,F10.6,A,F10.6)
  ...
  open(1001,file=datapath//'/case_c/stress_case_c2.csv')
  do i=1,T+1
  ...
  do i=1,T+1
     ...
     enddo
     g(i)=g(i)-conjg(Ak(N+1+1))*Fk(N+1+1)+(0,1)*(kappa*Ak(N)+Bk(N+1))*Fk(N+1+1)
     do k=2,N
  ...
  open(1002,file=datapath//'/case_c/displacement_case_c2.csv')
  do i=1,T+1
  ...
  ! enddo
  write(*,*) 'Horizontal rigid-body displacement of Case C Sol 2 =', ux(T)*1e1
  write(*,*) 'Vertical rigid-body displacement of Case C Sol 2 =', uy(T)*1e1
